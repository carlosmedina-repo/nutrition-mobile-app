...
const FoodsScreen = () => {
  ...
  const { resultsFromCamera } = useLocalSearchParams();

  const debouncedSearchTerm = useDebounce(searchTerm, 500);

  const { results, isLoading, fetchNextPage, hasNextPage, isError } =
    useFoodSearch({ searchTerm: debouncedSearchTerm, initialData: resultsFromCamera ? JSON.parse(resultsFromCamera as string) : undefined });

  useScrollToTop(flashListRef);

  useEffect(() => {
    if (resultsFromCamera) {
      setSearchTerm("Foods detected from camera");
    }
  }, [resultsFromCamera]);
